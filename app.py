...
st.info(
    """
    **Hướng dẫn:** Không có đề bài cố định trong ứng dụng.
...
if tu_dong_gia_han:
    cac_truong_hop.append("Không rút khi đến hạn / tự động gia hạn")
...
if co_nhu_cau_tien:
    cac_truong_hop.append("Khách hàng có nhu cầu tiền")
...
if rut_truoc_han:
    ...
    st.divider()
    st.subheader("📉 Xử lý rút trước hạn")
...
if rut_dung_han:
    ...
    st.divider()
    st.subheader("📈 Xử lý rút đúng hạn")
    ...
    st.success(
        ...
        **Lãi suất:** {lai_suat_co_ky_han:.2f}%/năm
        """
    )
...
if rut_sau_han:
    ...
    st.divider()
    st.subheader("🔄 Xử lý rút sau ngày đáo hạn")
    ...
    if ngay_rut_sau <= ngay_dao_han_ban_dau:
        ...
        so_ngay_sau_han = (
            ngay_rut_sau - ngay_dao_han_ban_dau
        ).days
        ...
        with col2:
            st.metric(
                "💵 Lãi sau đáo hạn",
                dinh_dang_tien(lai_sau_han)
            )

        with col3:
            st.metric(
                "🏦 Tổng tiền nhận",
                dinh_dang_tien(tong_sau_han)
            )
...
if nhan_lai_truoc:
    ...
    st.divider()
    st.subheader("💳 Xử lý nhận lãi trước")
...
if nhan_lai_hang_thang:
    ...
    st.divider()
    st.subheader("💳 Xử lý nhận lãi hàng tháng")
...
if nhan_lai_cuoi_ky:
    ...
    st.divider()
    st.subheader("💳 Xử lý nhận lãi cuối kỳ")
    ...
    with col1:
        st.metric(
            "💵 Tiền lãi",
            dinh_dang_tien(lai_cuoi_ky)
        )

    with col2:
        st.metric(
            "🏦 Gốc + lãi",
            dinh_dang_tien(tong_cuoi_ky)
        )
...
if tu_dong_gia_han:
    ...
    st.divider()
    st.subheader("🔄 Xử lý tự động gia hạn")
    ...
    for ky in range(1, int(so_ky_mo_phong) + 1):
        ...
        bang_gia_han.append(
            {
                "Kỳ": ky,
                "Ngày bắt đầu": ngay_bd_ky.strftime("%d/%m/%Y"),
                "Ngày đáo hạn": ngay_kt_ky.strftime("%d/%m/%Y"),
                "Số ngày": so_ngay_ky,
                "Tiền gốc đầu kỳ": round(goc_dau_ky),
                "Tiền lãi": round(lai_ky),
                "Tiền gốc sau gia hạn": round(tien_goc_hien_tai)
            }
        )
    ...
    with col1:
        st.metric(
            "💵 Tổng lãi các kỳ",
            dinh_dang_tien(tong_lai_gia_han)
        )

    with col2:
        st.metric(
            "🏦 Gốc sau mô phỏng",
            dinh_dang_tien(tien_goc_hien_tai)
        )
...
if co_nhu_cau_tien:
    ...
    st.divider()
    st.subheader("💰 Xử lý nhu cầu cần tiền")
...
if co_vay_cam_co:
    ...
    st.divider()
    st.subheader("🏦 Xử lý vay cầm cố sổ tiết kiệm")
...
if cac_truong_hop:
    ...
    st.divider()
    st.subheader("📑 Tóm tắt trường hợp đã chọn")

    df_tom_tat = pd.DataFrame(
        {
            "STT": range(1, len(cac_truong_hop) + 1),
            "Trường hợp": cac_truong_hop
        }
    )
